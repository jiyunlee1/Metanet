CLASS zcl_mcdli_cust_vh_re_posting DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MCDLI_CUST_VH_RE_POSTING IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_result TYPE TABLE OF zmcdli_u_vh_re_posting.
    ...
    TRY.
        ...
        lo_entity_list_resource = lo_client_proxy->create_resource_for_entity_set( 'A_RECONTRPOSTINGTERM' ).
    ...
    DATA lr_partner TYPE RANGE OF string.

    LOOP AT lt_result INTO DATA(wa_result).
      APPEND INITIAL LINE TO lr_partner[] ASSIGNING FIELD-SYMBOL(<fs_partner>).
      <fs_partner>-sign = 'I'.
      <fs_partner>-option = 'EQ'.
      <fs_partner>-low = wa_result-businesspartner.
    ENDLOOP.

    SELECT *
      FROM @lt_result AS rslt
     INNER JOIN i_businesspartner AS bp ON rslt~businesspartner = bp~businesspartner
