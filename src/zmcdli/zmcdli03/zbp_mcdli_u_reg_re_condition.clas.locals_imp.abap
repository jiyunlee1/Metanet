CLASS lhc_zmcdli_u_reg_re_condition DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zmcdli_u_reg_re_condition RESULT result.

    METHODS read FOR READ
      IMPORTING keys FOR READ zmcdli_u_reg_re_condition RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK zmcdli_u_reg_re_condition.

    METHODS valid_check FOR READ
      IMPORTING keys FOR FUNCTION zmcdli_u_reg_re_condition~valid_check RESULT result.

    METHODS create_condition FOR MODIFY
      IMPORTING keys FOR ACTION zmcdli_u_reg_re_condition~create_condition RESULT result.

ENDCLASS.

CLASS lhc_zmcdli_u_reg_re_condition IMPLEMENTATION.
  ...
  METHOD valid_check.
    TYPES: BEGIN OF ts_condition,
             internalrealestatenumber  TYPE zmcdli_u_reg_re_condition-internalrealestatenumber,
             restatusobjectcalculation TYPE zmcdli_u_reg_re_condition-restatusobjectcalculation,
             reconditiontype           TYPE zmcdli_u_reg_re_condition-reconditiontype,
             reextconditionpurpose     TYPE zmcdli_u_reg_re_condition-reextconditionpurpose,
             validitystartdate         TYPE zmcdli_u_reg_re_condition-validitystartdate,
             validityenddate           TYPE zmcdli_u_reg_re_condition-validityenddate,
             repostingterm             TYPE zmcdli_u_reg_re_condition-repostingterm,
             rerhythmterm              TYPE zmcdli_u_reg_re_condition-rerhythmterm,
             reorglassignmentterm      TYPE zmcdli_u_reg_re_condition-reorglassignmentterm,
             recalculationrule         TYPE zmcdli_u_reg_re_condition-recalculationrule,
             reunitprice               TYPE zmcdli_u_reg_re_condition-reunitprice,
             reconditioncurrency       TYPE zmcdli_u_reg_re_condition-reconditioncurrency,
             localpath                 TYPE zmcdli_u_reg_re_condition-localpath,
           END OF ts_condition.
...
*   필수 입력 키 체크
    LOOP AT lt_condition INTO ls_condition.
      IF ls_condition-reunitprice IS INITIAL OR ls_condition-validitystartdate IS INITIAL OR ls_condition-validityenddate IS INITIAL.
...
*   local 중 중복된 key
    ...
    DATA: lt_filter_val TYPE TABLE OF ts_filter_val,
          lr_period     TYPE RANGE OF string,
          lr_nowperiod  TYPE RANGE OF string,
          lv_count      TYPE i.

    LOOP AT lt_condition INTO ls_condition.
      APPEND INITIAL LINE TO lt_filter_val ASSIGNING FIELD-SYMBOL(<fs_filter_val>).
      <fs_filter_val> = CORRESPONDING #( ls_condition ).
      lr_period = VALUE #( ( sign = 'I' option = 'BT' low = ls_condition-validitystartdate high = ls_condition-validityenddate ) ).
      CLEAR lv_count.
      SELECT * FROM @lt_condition AS cond
             WHERE cond~internalrealestatenumber = @ls_condition-internalrealestatenumber
              ...
              INTO @DATA(ls_comptrgt).
        lr_nowperiod = VALUE #( ( sign = 'I' option = 'BT' low = ls_comptrgt-validitystartdate high = ls_comptrgt-validityenddate ) ).
        IF ls_comptrgt-validityenddate IN lr_period OR ls_comptrgt-validityenddate IN lr_period OR ls_condition-validitystartdate IN lr_nowperiod.
          lv_count += 1.
        ENDIF.
        IF LV_COUNT > 1.
          APPEND INITIAL LINE TO result[] ASSIGNING FIELD-SYMBOL(<fs_result2>).
          <fs_result2>-%param-localpath = ls_condition-localpath.
          <fs_result2>-%param-errmsg = 'Local Data 에 겹치는 Validity 기간이 존재합니다.'.
          EXIT.
        ENDIF.
      ENDSELECT.
    ENDLOOP.
...
*   기존 데이터와 비교
    ...
    TYPES: BEGIN OF ts_existing,
             ...
             reextconditionpurpose     TYPE zmcdli_u_reg_re_condition-reextconditionpurpose,
             validitystartdate         TYPE zmcdli_u_reg_re_condition-validitystartdate,
             validityenddate           TYPE zmcdli_u_reg_re_condition-validitystartdate,
             repostingterm             TYPE zmcdli_u_reg_re_condition-repostingterm,
             rerhythmterm              TYPE zmcdli_u_reg_re_condition-rerhythmterm,
             reorglassignmentterm      TYPE zmcdli_u_reg_re_condition-reorglassignmentterm,
             recalculationrule         TYPE zmcdli_u_reg_re_condition-recalculationrule,
...
*     One Time
      ...
      SELECT SINGLE * FROM @lt_existing AS cond
       WHERE cond~internalrealestatenumber = @ls_condition-internalrealestatenumber
         AND cond~reconditiontype = @ls_condition-reconditiontype
         AND cond~validitystartdate IN @lr_period
...
*     ELSE
      ...
      SELECT * FROM @lt_existing AS cond
       WHERE cond~internalrealestatenumber = @ls_condition-internalrealestatenumber
         ...
         INTO @ls_existing.
        CLEAR lr_range.
        lr_nowperiod = VALUE #( ( sign = 'I' option = 'BT' low = ls_existing-validitystartdate high = ls_existing-validityenddate ) ).
        IF ls_existing-validityenddate IN lr_period OR ls_existing-validityenddate IN lr_period OR ls_condition-validitystartdate IN lr_nowperiod.
          APPEND INITIAL LINE TO result[] ASSIGNING FIELD-SYMBOL(<fs_result4>).
          <fs_result4>-%param-localpath = ls_condition-localpath.
          <fs_result4>-%param-errmsg = '기존 데이터에 겹치는 Validity 기간이 존재합니다.'.
          EXIT.
        ENDIF.
      ENDSELECT.
    ENDLOOP.
...
CLASS lsc_zmcdli_u_reg_re_condition DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zmcdli_u_reg_re_condition IMPLEMENTATION.
