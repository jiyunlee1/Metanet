CLASS zcl_mcdar_cust_act_mnt DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MCDAR_CUST_ACT_MNT IMPLEMENTATION.
...
*          ls_comb_base TYPE ts_smst,
      ...
      lv_smst     TYPE zmcdar_i_act_mnt_src-smartstore.

    FIELD-SYMBOLS: <fv_smst> TYPE zmcdar_i_act_mnt_src-smartstore.
...
*    ENDSELECT.
    ...
    DATA: lv_store   TYPE zmcdar_i_act_mnt_src-storecode,
          lv_subsite TYPE zmcdar_i_act_mnt_src-subsite,
          lv_nowamt  TYPE zmcdar_i_act_mnt_src-mntlyamt,
          lv_err     TYPE c LENGTH 1300.

    DATA lt_result TYPE TABLE OF zmcdar_u_act_mnt.

*    lt_result = CORRESPONDING #( lt_actmnt ).
    ...
    SELECT
      ...
      currency,
      mntlyamt
    FROM zmcdar_i_act_mnt_src
    WHERE fiscalyear   = @lv_year
      AND fiscalperiod = @lv_period
    INTO CORRESPONDING FIELDS OF TABLE @lt_result.

    LOOP AT lt_base INTO DATA(ls_base).
      CLEAR: lv_store, lv_subsite, lt_comb.
      DATA: lv_idx TYPE i,
            lv_len TYPE i.
...
*      inner join zmcdar_i_act_mnt_src AS b
...
*        @data(ls_comb_proc).
      ...
      WHILE lv_idx LE lv_len.
        ...
        IF ls_now-iscomb = 'X'.
          APPEND INITIAL LINE TO lt_result ASSIGNING FIELD-SYMBOL(<fs_actmnt>).
          <fs_actmnt>-fiscalyear = lv_year.
          <fs_actmnt>-fiscalperiod = lv_period.
          <fs_actmnt>-storecode    = lv_store.
          <fs_actmnt>-subsite      = lv_subsite.
          <fs_actmnt>-smartstore   = ls_comb-smartstore.
          <fs_actmnt>-sourceledger = '0L'.
          <fs_actmnt>-companycode  = '1000'.
          <fs_actmnt>-ledger       = '0L'.
          <fs_actmnt>-profitcenter = ls_base-profitcenter.
          <fs_actmnt>-glaccount = 0.
          <fs_actmnt>-glaccountname = ''.
          <fs_actmnt>-currency     = 'KRW'.
          <fs_actmnt>-mntlyamt      = lv_nowamt.
          <fs_actmnt>-cnt          = 0.
          <fs_actmnt>-iscomb       = ls_comb-iscomb.
          <fs_actmnt>-formula      = ls_comb-formula.
          <fs_actmnt>-errmsg       = ls_now-errmsg.
        ENDIF.
